...
LacCurveFit<-function(data,ID,trait,dim,alpha=0.5, models="All",param_list=NULL){
  ...
  for(i in ids){
    ...
    out.ModelsLac<-ModelsLac(data=x,ID_col=ID,ID=i, trait=trait, dim=dim, alpha=alpha,models=models,param_list=param_list)
